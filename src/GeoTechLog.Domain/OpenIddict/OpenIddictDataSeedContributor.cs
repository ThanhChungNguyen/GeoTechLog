...
namespace GeoTechLog.OpenIddict;
...
public class OpenIddictDataSeedContributor : IDataSeedContributor, ITransientDependency
{
    ...
    {
        if (await _openIddictScopeRepository.FindByNameAsync("GeoTechLog") == null)
        {
            await _scopeManager.CreateAsync(new OpenIddictScopeDescriptor {
                Name = "GeoTechLog", DisplayName = "GeoTechLog API", Resources = { "GeoTechLog" }
            });
        }
    }
    ...
    private async Task CreateApplicationsAsync()
    {
        ...
        //Web Client
        var webClientId = configurationSection["GeoTechLog_Web:ClientId"];
        if (!webClientId.IsNullOrWhiteSpace())
        {
            var webClientRootUrl = configurationSection["GeoTechLog_Web:RootUrl"]!.EnsureEndsWith('/');

            /* GeoTechLog_Web client is only needed if you created a tiered
            ...
            await CreateApplicationAsync(
                ...
                secret: configurationSection["GeoTechLog_Web:ClientSecret"] ?? "1q2w3e*",
        ...
        // Swagger Client
        var swaggerClientId = configurationSection["GeoTechLog_Swagger:ClientId"];
        if (!swaggerClientId.IsNullOrWhiteSpace())
        {
            var swaggerRootUrl = configurationSection["GeoTechLog_Swagger:RootUrl"]?.TrimEnd('/');
